...
21) Send a secret AUTH packet to a secret IP address of the service.
22) Update public document or file with secret word or symbol somewhere. As an example, user might use his github repo as additional auth step. He than might specify secret condition to use to succeed at this step. For example, he needs to add *bold text* saying "*Note:*" in specific place of specific file. Public document can be anything: RAW github file, RAW text file on your server, Google Documents public document, etc. etc. 
*) Additional "auth factors" can be proposed and added by users via GitLab/Github etc.

* Each auth factor can have text, audio, video, game, or everything both as hints or misleads/distractions.
...
* Some of the "AUTH STEPS" can be used as switches "ON/OFF". For example, #22 (Update public document) can be used to allow log-in into your system. Let's say you go on a vacation and before that, you don't want for ANYBODY to beable to log-in into your account. You remove the boldness from "*Note:*" text in your github or on your web server text file. While not having "bold" state, nobody can log-in into your account. While being on vacation, you decided that you need to login and check something out. You then change the file back to secretly required state, log-in and use your MetaPass account. So, in this case, external file is used as ON/OFF switch to allow or disallow login into your account.
